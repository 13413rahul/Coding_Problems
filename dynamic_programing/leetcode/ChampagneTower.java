...
public class ChampagneTower {
    double t[][]; // declaring 2 d array, to store some already computed results
    public double solve(int poured, int row, int glass)
    {
    ...
    double champagneTower(int poured, int query_row, int query_glass) {
        t = new double[101][101];
        for(int i = 0; i < 101; i++) {
            Arrays.fill(t[i], -1);
        }
        // solve funcion which return answer
        return Math.min(1.00, solve(poured, query_row, query_glass));
    }
}
